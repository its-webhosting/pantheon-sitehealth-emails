...
NOTICE_SITELENS_URL_PATHS = sc.registry.register(
...
def setup_sitelens(connection) -> None:
    ...
    global sitelens_configured_scans_by_site, sitelens_scores  # noqa: PLW0602 -- module-state bag idiom (CLAUDE.md); mutated in place below via subscript/append, not reassigned, so no `= ` is needed or wanted
    ...
    if sc.options.verbose >= 3:  # noqa: PLR2004 -- -vvv; the numeric verbosity levels are the CLI's own convention (CLAUDE.md), not a value worth naming here
...
def check_sitelens_urls(site_context) -> None:
    ...
    if num_paths_configured >= 4:  # noqa: PLR2004 -- SiteLens's minimum-configured-paths threshold; the human-facing "at least two" explanation lives in the notice text built below, not duplicated here as a named constant
...
def create_gauge_image(value: int, color: str, title: str) -> bytes:
    ...
    fig, ax = plt.subplots(figsize=(GAUGE_PIXELS_WIDTH * px, GAUGE_PIXELS_HEIGHT * px), subplot_kw={"aspect": "equal"})

    ax.pie([100], radius=1.19, wedgeprops={"width": 1.2}, colors=[BACKGROUND_GAUGE_COLOR])  # solid background color inside the gauge
    ax.pie(data, radius=1.2, wedgeprops={"width": 0.3}, counterclock=False, startangle=90, colors=[color, 'white'])
    ...
    left, b, w, h = ax.get_position().bounds
    ax.set_position([left, b - 0.1, w, h])  # move the gauge down a bit to balance with the title
...
def check_sitelens_scores(site_context) -> None:
    ...
    scan_count = 0
    timestamp = datetime.datetime.min  # noqa: DTZ901 -- sentinel compared only against portal-DB timestamps, which are naive too; attaching tzinfo here would break every comparison below without also retrofitting the DB read (a behavior change this task may not make)
    for path in paths:
        ...
        timestamp = max(timestamp, sitelens_scores[path]['timestamp'])
    ...
    for key in score_ordering:
        ...
        score[key]['value'] = round(100 * score[key]['value'] / scan_count)
