    ...
    if len(errors) > 0 and p.returncode == 0:
        ...
        for line in lines:
            ...
            if (
                ...
                and not "This environment is in read-only Git mode" in line
                and not (
...
def wp(siteenv: str, *args) -> (Any, str, bool):
    """
...
def wp_eval(siteenv: str, *args) -> (str, str, bool):
    """
...
def drush(siteenv: str, *args) -> (Any, str, bool):
    """
...
def drush_php_script(siteenv: str, script: str) -> (Any, str, bool):
    """
...
longer offer PHP {envs["live"]["php_version"]} soon</a>, likely sometime in 2027.</p>
""",
                        "text": f"""
...
since Pantheon has announced they will no longer offer PHP {envs["live"]["php_version"]}
soon</a>, likely sometime in 2027.
<https://docs.pantheon.io/release-notes/2026/03/php-removal-schedule>
""",
                    }
                )
            elif envs["live"]["php_version"] < "8.2":
                new_php = "7.4" if envs["live"]["php_version"].startswith("7") else "8.1"
                site_context.add_notice(
                    {
                        "type": "alert",
                        "icon": "&#x1F6A8;",  # police car light
...
""",
            ...
            if end_of_contract_year and umich_enabled():
                ...
                <p style="background-color: #f8d7da; padding: 1em; border: 2px solid #58151c;">
                    ...
                    full year (July 1 - June 30) of Pantheon hosting on the {site["plan_name"]} plan for the site
                    <strong>{site["name"]}</strong>.
                </p>
                <p>Please see if a different plan would be better:</p>
                <ul>
                    <li><a href="#estimated-costs">Estimated Plan Costs for {site["name"]}</a> (see the table, below)</li>
                ...
                <ul>
                    <li>
                        <a href="https://admin.webservices.umich.edu/sites/{portal_site_id}/plan/">Change the plan for {site["name"]}</a>.
                        Changes must be made by the end of the day on June 29 for the July 1 annual billing.
                    </li>
                    <li>
                        <a href="https://admin.webservices.umich.edu/sites/{portal_site_id}/edit/">Change the shortcode for {site["name"]}</a>.
...
{site["plan_name"]} plan for the site {site["name"]}.
=======================================================================

Please see if a different plan would be better:

  * See the Estimated Plan Costs for {site["name"]}
    in the table below.
...
""",
            ...
            if umich_enabled():
                ...
                <p style="background-color: #f8d7da; padding: 1em; border: 2px solid #58151c;">
                    ...
                    for a Pantheon {site["plan_name"]} plan to cover website hosting for the site
                    <strong>{site["name"]}</strong> from July 1, 2026 - June 30, 2027.
                </p>
...
for a Pantheon {site["plan_name"]} plan to cover website hosting
for the site {site["name"]} from July 1, 2026 - June 30, 2027.
=======================================================================
